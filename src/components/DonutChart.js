...
class DonutChart extends React.Component {
  ...
  _animateActiveArc = (currentActiveIndex, nextActiveIndex) => {
    ...
    if (currentArcData) {
      d3.select(this['arc-' + this.props.id + currentActiveIndex])
    ...
    if (nextArcData) {
      d3.select(this['arc-' + this.props.id + nextActiveIndex])
        .transition('nextArc')
        .duration(200)
  ...
  _renderDataLabel = (styles, colors) => {
    if (this.props.showDataLabel) {
      if (this.props.children) {
        ...
        const activeAndDataLabelHover =
          this.state.activeIndex !== -1 && this.props.shouldToggleDataLabelOnHover
        const text = activeAndDataLabelHover ? activeDataSet.name : this.props.defaultLabelText
        const value = this.props.formatter(
          activeAndDataLabelHover ? activeDataSet.value : this.props.defaultLabelValue,
        )
  ...
  render() {
    ...
    const dataPointColors =
      this.props.dataPointColors || [theme.Colors.SUCCESS].concat(d3.scale.category20b().range())
    ...
    return (
      ...
      >
        ...
        >
          <g {...this.props.elementPropsForChartG} className="mx-donutchart-g" transform={position}>
            {this._renderBaseArc(baseArcColor)}
            {this._renderArcs(styles, colors)}
