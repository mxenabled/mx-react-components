...
class FileUpload extends React.Component {
  ...
  _getDefaultValidationMessage(validationType) {
    ...
    switch (validationType) {
      case 'exact_height':
        validationMessage =
          'Image requires an exact height of ' + this.props.imageValidation.exactHeight + ' pixels'
        break
      case 'exact_width':
        validationMessage =
          'Image requires an exact width of ' + this.props.imageValidation.exactWidth + ' pixels'
        break
      case 'file_size':
        validationMessage =
          'File size is too large. Must be ' + this.props.maxFileSize + 'k or less.'
        break
      case 'file_type':
        validationMessage =
          'Unpermitted File Type. Must be one of: ' + this.props.allowedFileTypes.join(', ')
        break
      case 'image_ratio':
        validationMessage =
          'Image ratio must be ' +
      ...
      case 'max_height':
        validationMessage =
          'Maximum Image height of ' + this.props.imageValidation.maxHeight + ' exceeded'
        break
      case 'max_width':
        validationMessage =
          'Maximum Image width of ' + this.props.imageValidation.maxWidth + ' exceeded'
        break
      case 'min_height':
        validationMessage =
          'Image must have minimum heigh of ' + this.props.imageValidation.minHeight + ' pixels'
        break
      case 'min_width':
        validationMessage =
          'Image must have minimum width of ' + this.props.imageValidation.minWidth + ' pixels'
        break
      default:
        break
    }
  ...
  _validateImageDimensions = (width, height) => {
    ...
    if (this.props.imageValidation) {
      ...
      if (
        this.props.imageValidation.ratioHeight &&
        this.props.imageValidation.ratioWidth &&
        this.props.imageValidation.ratioHeight / this.props.imageValidation.ratioWidth !==
          height / width
      ) {
  ...
  _validateFile = (file, width = null, height = null) => {
    ...
    const isInvalidFileType =
      this.props.allowedFileTypes && this.props.allowedFileTypes.indexOf(file.type) < 0
    const isInvalidFileExtension =
      this.props.allowedFileTypes && this.props.allowedFileTypes.indexOf(fileExt) < 0
