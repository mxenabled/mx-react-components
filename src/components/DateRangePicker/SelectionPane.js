...
class SelectionPane extends React.Component {
        ...
        <div>
          ...
          <a
            aria-label={`Select Start Date, ${
              selectedStartDate
                ? 'Current start date is ' + moment.unix(selectedStartDate).format('MMM D, YYYY')
                : ''
            }`}
            ...
            style={Object.assign(
              ...
              this.props.selectedBox === SelectedBox.FROM ? styles.selectedDateSelectBox : null,
            )}
            tabIndex={0}
          >
            {selectedStartDate
          ...
          <a
            aria-label={`Select End Date, ${
              selectedEndDate
                ? 'Current end date is ' + moment.unix(selectedEndDate).format('MMM D, YYYY')
                : ''
            }`}
            onClick={() => this._handleDateBoxClick(selectedEndDate, SelectedBox.TO)}
            onKeyUp={e =>
              keycode(e) === 'enter' && this._handleDateBoxClick(selectedEndDate, SelectedBox.TO)
            }
            role="button"
            style={Object.assign(
              {},
              styles.dateSelectBox,
              this.props.selectedBox === SelectedBox.TO ? styles.selectedDateSelectBox : null,
            )}
            tabIndex={0}
          >
            {selectedEndDate
  ...
  styles = theme => {
    const isLargeOrMediumWindowSize =
      ['large', 'medium'].indexOf(StyleUtils.getWindowSize(theme.BreakPoints)) !== -1
