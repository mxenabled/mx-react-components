  ...
  render() {
    return (
      <div>
        ...
        <p>
          An array of objects that will populate the button values. Works with as little as one
          object. Objects take an <Code>aria-label</Code>, <Code>icon</Code>, <Code>text</Code>, and{' '}
          <Code>style</Code>.
        </p>
        <p>
          A button can be disabled by adding <Code>type = 'disabled'</Code> but no other button
          types are supported within the button group.
        </p>
        <p>
          The <Code>aria-label</Code> attribute for each button is used for accessibility purposes
          but is not required. See the <Link to="/components/button">Button</Link> component
